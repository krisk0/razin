...
# This file is a fork of flint-2.4.3.ebuild found at RAZIN git 
#  (https://github.com/krisk0/razin)

# Main difference from flint-2.4.3.ebuild: bland extension is built
#  when ext flag is set

# Experimentation shows that make voodoo 'foreach' only works if double-quotes 
#  are not present in expression --extensions=
...
src_configure() {
 ...
 use ext && with_ext="--extensions=$WORKDIR/bland-master"

 # configure dislikes "$empty_string", two potentially empty strings are here:
 #  $with_ntl and $with_ext.
 # $with_ntl is moved to bottom, and branch below ensures that no other empty 
 #  strings are present
 use ext && 
  {
   ./configure \
