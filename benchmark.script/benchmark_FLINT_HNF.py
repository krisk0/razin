...
'''
This program tests/benchmarks my re-implementation of Sage hnf_square() 
 against Alex Best subroutine fmpz_mat_hnf() (part of FLINT)
...
def do_benchmark( m ):
 ...
 t0=time.time()
 sage_r=reimplemented_hnf_square(m)
 t1=time.time()
 m._clear_cache()
 mine_r=AlexBest( fmpz_mat(m) ).export_sage()
 t2=time.time()
 if mine_r != sage_r:
  print 'result mismatch, m=\n',m
  print 'Д Крыськов result=\n',sage_r
  print 'Alex Best  result=\n',mine_r
  sys.exit(1)
 t0=t1-t0
 if t0<t_sage_min:
  t_sage_min=t0
...
def benchmark( dim, bits, tries, experiment_no ):
 ...
 print 'n=%s bits=%s Python/C=%.2f/%.2f' % (dim,bits,avg_sage_time,avg_mine_time)
...
if __name__ == "__main__":
 sage.all.set_random_seed('20141003')
 ...
 for i in dim_data:
  ...
  for j in bits_choice:
   if j >= i[1] and j <= i[2]:
    ...
    if not debug_mode:
     print 'n=%s bits=%s max time Python/C=%s/%s' % (i[0],j,t_sage_max,\
      t_mine_max)
     sys.stdout.flush()
  result_row += 1
